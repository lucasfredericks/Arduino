...
float dividerMod = (R2) / (R1 + R2); //multiply this value by voltage after the divider to calculate voltage before the divider
float voltage;
float lastVoltage; //comparison value to reduce noise
string wmid = "WM4/"; //CHANGE THIS TO MATCH THE WINDMILL IDENTITY! Everything else can stay the same between windmills.
...
void loop() {
  //measure the voltage at analog pin 1 and convert for the voltage divider.
  voltage = analogRead(A1);
  voltage = (voltage / 1024.0) * 3.3;
  voltage = voltage * dividerMod;
//  if (voltage < 0.02) {
//    voltage = 0;
//  }
